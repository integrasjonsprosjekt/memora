...
func (s *CardService) GetCardInDeck(
	...
	if err := s.cache.Get(ctx, cacheKey, &rawCard); err == nil {
		card, err := GetCardStruct(rawCard, fmt.Errorf("internal server error"))
		if err == nil {
			return card, nil
		}
	}
	...
	// Cache for next time
	s.cache.SetAsync(cacheKey, raw, CardTTL)
